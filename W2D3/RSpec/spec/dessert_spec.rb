...
describe Dessert do
  ...
  describe "#initialize" do
    ...
    it "raises an argument error when given a non-integer quantity" do
      expect { Dessert.new('cake', 5.5, 'chef') }.to raise_error(ArgumentError)
    end
  end

  describe "#add_ingredient" do
    it "adds an ingredient to the ingredients array" do
      expect(cake.ingredients).to_not include('milk')
      cake.add_ingredient('milk')
      expect(cake.ingredients.last).to eq('milk')
    end
  end

  describe "#mix!" do
    it "shuffles the ingredient array" do
      ingredients = ['milk', 'cocoa', 'sugar', 'egg']
  ...
  describe "#serve" do
    it "contains the titleized version of the chef's name" do
      expect(chef).to receive(:titleize).and_return("Chef Bao the Great Baker")
      expect(cake.serve).to eq("Chef Bao the Great Baker has made 5 cakes!")
    end
  end
